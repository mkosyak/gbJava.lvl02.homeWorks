...
//public class Cat implements Subjects, Runnable, HJumpable {   // -- Dummy: quick exclude interface
public class Cat implements Subjects, Runnable, HJumpable, LJumpable {
    public static final String objType = "Cat";
    private String objName;
    private float totalRunDist = 0.0F;     //meters - Cumulative value
    private float maxRunDist = 20.0F;      //meters
    private float maxJumpHeight = 2.5F;    //meters
    private float maxJumpDist = 5F;        //meters
    ...
    public Cat(String actName) {
        objName = actName;
        System.out.printf("%n:> Participant %s (type %s) is assigning with the *DFT characters:", objName, objType);
        initMaxValues(maxRunDist, maxJumpHeight, maxJumpDist);
    }

    public Cat(String actName, float run, float hJump, float lJump) {
        objName = actName;
